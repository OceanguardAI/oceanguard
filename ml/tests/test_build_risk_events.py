...
def test_build_events_returns_expected_shape(tmp_path, monkeypatch):
    ...
    events = builder.build_events(source_root=tmp_path, output_dir=outputs_dir)
    ...
    assert set(builder.RISK_EVENT_FIELDS).issubset(events[0].keys())


def test_build_events_supports_temporary_artifact_layout(tmp_path):
    source_root = tmp_path / "Temprary" / "ml"
    data_dir = source_root / "data"
    outputs_dir = source_root / "outputs"
    target_output_dir = tmp_path / "generated"
                ...
                "elements": [
                    {
                        "lat": 8.2155202,
                        "lon": 79.7061466,
                        "tags": {"name": "Marina near Bar Reef"},
                    }
                ]
            }
        ),
    ...
    events = builder.build_events(source_root=source_root, output_dir=target_output_dir)
...
def test_build_events_reports_when_gfw_fallback_data_is_used(tmp_path):
    data_dir = tmp_path / "data"
    outputs_dir = tmp_path / "outputs"
    data_dir.mkdir()
    outputs_dir.mkdir()
    ...
    events, metadata = builder.build_events(
        source_root=tmp_path,
        output_dir=outputs_dir,
        return_metadata=True,
    )

    assert metadata["used_fallback_gfw_data"] is True
    assert events[0]["id"] == "bar-reef-001"
