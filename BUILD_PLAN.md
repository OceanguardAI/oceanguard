...
Live ingestion (`backend/app/services/gfw_ingest.py::_score_detection`) scores
each SAR detection additively — fully transparent, no black box:

```python
score = 0.25                       # baseline: any SAR vessel detection
if not ais_matched:    score += 0.20   # no AIS identity — possible dark vessel
if   distance_to_mpa_km <= 0:  score += 0.45   # inside an MPA
elif distance_to_mpa_km <= 10: score += 0.30   # near the boundary
elif distance_to_mpa_km <= 50: score += 0.15   # surrounding waters
if detections > 1:     score += 0.05   # repeated presence at the same cell
score = min(score, 0.99)

# Thresholds
CRITICAL ≥ 0.80 | HIGH ≥ 0.60 | MEDIUM ≥ 0.45 | LOW < 0.45

# YOLO agreement boost (officer's on-demand YOLO check confirms a GFW detection)
risk_score = min(0.99, risk_score + 0.10)
```

**Key thresholds:**
- Near MPA: ≤ 10 km from boundary (`mpa_index.NEAR_MPA_KM`)
- Detection confidence: ≥ 0.45 to pass ingestion filter

> The offline seed `risk_events.json` (no-token fallback) was generated by
> `ml/pipeline/risk.py`, a weighted variant with `bar-reef-NNN` ids. A deployed
> instance with `GFW_API_TOKEN` uses the additive formula above.
